    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests((requests) -> requests
                        // requestMatchers(...).permitAll(): 指定されたURLパターンに対するリクエストをすべて許可します（認証不要）。
                        ...
                        .permitAll()
                        // requestMatchers("/admin/**").hasRole("ADMIN"): "/admin/" で始まるURLパターンへのアクセスは、
                ...
                .headers((headers) -> headers
                        ...
                        .contentSecurityPolicy((csp) -> csp
                                ...
                                // script-src 'self' ...: スクリプトは自分自身、Google関連ドメイン、CDN、インラインスクリプトを許可。
                                // style-src 'self' ...: スタイルは自分自身、Google Fonts、CDN、インラインスタイルを許可。
                                // img-src 'self' data:: 画像は自分自身、data URIスキームを許可。
                                // font-src 'self' ...: フォントは自分自身、Google Fonts、CDNを許可。
                                // frame-src 'self' https://www.google.com: フレームは自分自身、Googleドメインを許可（reCAPTCHA用）。
                        ...
                        .xssProtection((xss) -> xss
                                // headerValue(...): ヘッダー値を設定。ENABLED_MODE_BLOCK はXSS検出時にページのレンダリングを停止します。
                                .headerValue(XXssProtectionHeaderWriter.HeaderValue.ENABLED_MODE_BLOCK))
                        // contentTypeOptions: X-Content-Type-Options ヘッダーを設定します (デフォルトで 'nosniff'
                        // が設定されます)。
